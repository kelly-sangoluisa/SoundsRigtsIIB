...
export default function Dashboard() {
  ...
  const convertSongFormat = (song: Song) => {
    return {
      ...
      owner: song.ownerId,
      currentOwnerId: song.currentOwnerId,
      status: song.status,
  ...
  const handleRequestSong = async (songId: string) => {
    ...
    try {
      console.log('🛒 Solicitando compra de canción:', songId);
      await songsService.requestSong(songId, user.id);
      alert('¡Solicitud de compra enviada! El dueño de la canción será notificado.');
      
      // Recargar las canciones para mostrar el nuevo estado
      loadSongs();
    } catch (error) {
      console.error('Error al solicitar canción:', error);
      alert('Error al solicitar la canción. Inténtalo de nuevo.');
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredSongs.map((song) => {
            const convertedSong = convertSongFormat(song);
            const isOwner = song.currentOwnerId === user?.id || song.ownerId === user?.id;
